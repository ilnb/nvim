---@param buf integer
local function reload_workspace(buf)
  local clients = vim.lsp.get_clients { bufnr = buf, name = 'rust_ls' }
...
local function user_sysroot_src()
  return vim.tbl_get(vim.lsp.config['rust_ls'], 'settings', 'rust-analyzer', 'cargo', 'sysrootSrc')
end

local function default_sysroot_src()
  local sysroot = vim.tbl_get(vim.lsp.config['rust_ls'], 'settings', 'rust-analyzer', 'cargo', 'sysroot')
  if not sysroot then
    ...
    if result.code == 0 and stdout then
      if stdout:sub(#stdout) == '\n' then
        if #stdout > 1 then
          sysroot = stdout:sub(1, #stdout - 1)
        else
          sysroot = ''
        end
      else
...
local function is_library(fname)
  ...
  local cargo_home = os.getenv 'CARGO_HOME' or home .. '/.cargo'
  local registry = cargo_home .. '/registry/src'
  local git_registry = cargo_home .. '/git/checkouts'

  local rustup_home = os.getenv 'RUSTUP_HOME' or home .. '/.rustup'
  ...
  for _, item in ipairs { toolchains, registry, git_registry, sysroot_src } do
    if item and vim.fs.relpath(item, fname) then
      local clients = vim.lsp.get_clients { name = 'rust_ls' }
    ...
    vim.system(cmd, { text = true }, function(output)
      if output.code == 0 then
        ...
        vim.schedule(function()
          vim.notify(('[rust_ls] cmd failed with code %d: %s\n%s'):format(output.code, cmd, output.stderr))
        end)
      end
    end)
  end,
